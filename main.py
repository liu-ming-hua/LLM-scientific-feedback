...
class DeepSeekWrapper:
    ...
    def send_query(self, user_str, n_query=1):
        print(f"# tokens sent to DeepSeek: {self.compute_num_tokens(user_str)}")
        query_args = self.make_query_args(user_str, n_query)
...
wrapper = DeepSeekWrapper(model_name="deepseek-ai/DeepSeek-V3")
...
def extract_element_text(element):
    ...
    This function recursively traverses the element tree, concatenating the text content of each element and its children.
...
def prompt_function_truncated_full_paper(parsed_xml: Dict, user_content: str):
...
My review is that: {user_content}.

Please convert my review into the following format

Start by "Review outline:".
And then:
"1. My first point in review: find the corresponding place in the paper."
"2. My second point in review: find the corresponding place in the paper."
"3. My third point in review: find the corresponding place in the paper."
“.etc”
Recommand some related papers of this paper.
...
def step1_get_xml(input_file: str) -> str:
    ...
    convert_url = "http://localhost:6090/api/convert"
...
def step3_get_lm_review(parsed_xml: Dict, user_content: str) -> Dict:
    text_to_send = prompt_function_truncated_full_paper(parsed_xml, user_content)
...
def process(file_content, user_content):
    ...
    try:
        ...
        review_generated = step3_get_lm_review(parsed_xml, user_content)
    except Exception as e:
...
def main():
    temp_dir = "/home/zhanglj2023/lmh/LLM-scientific-feedback/tmp/dir"

    os.environ["GRADIO_TEMP_DIR"] = temp_dir

    upload_component = gr.File(label="Upload PDF", type="binary")
    user_input_component = gr.Textbox(label="User Input", placeholder="Enter your input here...")
    output_component_review = gr.Textbox(label="Review Generated")

    demo = gr.Interface(
        fn=process, inputs=[upload_component, user_input_component], outputs=output_component_review
    )
